...
def main(topic_name=None, difficulty=None):

    game = GameState(topic=None, difficulty=None, word=None) 

    
    if topic_name is None:
        topic_name = input("Choose a topic (plants, animals, sports): ").strip().lower()
        while topic_name not in topicdict:
            topic_name = input("Invalid topic. Choose again: ").strip().lower()

    
    if difficulty is None:
        difficulty = input("Choose a difficulty (easy, medium, hard): ").strip().lower()
        while difficulty not in ["easy", "medium", "hard"]:
            difficulty = input("Invalid difficulty. Choose again: ").strip().lower()

    print(f"Topic: {topic_name}")
    print(f"Difficulty: {difficulty}")

    
    words_for_topic = topicdict[topic_name]
    num_words = {"easy": 5, "medium": 8, "hard": 12}[difficulty]
    selected_words = random.sample(words_for_topic, min(num_words, len(words_for_topic)))

    grid = Grid(rows=10, columns=10)
    grid.place_words(selected_words)
    grid.fill_random()
    ...
    valid_words = set(w.lower() for w in selected_words)

    
    print("\nStart guessing! Type 'DONE' when finished.\n")
    while True:
        guess = input("Enter a word: ").strip()
        if guess.upper() == "DONE":
            break
        is_valid, message = input_validation(guess, valid_words, game.guessed_words)
        print(message)
        if is_valid:
            game.score += 1
